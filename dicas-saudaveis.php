...
<title>AsaLight - Dicas Saud&aacute;veis</title>
...
<link rel="stylesheet" href="css/corpo_dicas_saudaveis_style.css" type="text/css" media="all" />
...
e promoções da ASA Light.</td>
    ...
    <div class="corpo_um">
    	<div class="corpo_um_margin">
    		<div id="img_corpo_um">
            	<img src="img/img_nossaloja.png" alt="dicas" />
            </div>
            <div id="txt_corpo_um">
            	<img src="img/txt_dicas.png" alt="txt dicas" />
            </div>
            <div id="conteudo">
            	<table>
                	<tr>
                    	<td><font style="font-size:23px; color:#963e52;">Nossa Loja</font>
                        <p>
                        A ASA LIGHT possui  um espaço diferenciado, que dispõe de:<p>



<B>1.</B> Produtos organicos,  naturais e suplementos;<p>
<B>2.</B> Saborosas refeições congeladas servidas no local;<p>
<B>3.</B> Dicas saudáveis;<p>
<B>4.</B> Nutricionistas calculando seu Indice de Massa Corporal e auxiliando a escolha de seu Programa Nutricional;<p>
<B>5.</B> Consultório - Agende sua consulta e tenha um acompanhamento, com:<br>
5.1 - Desenvolvimento de dietas personalizadas;<br />
5.2 - Avaliação Física (Percentual de gordura)<br />
5.3 - Avaliação por BIOIMPEDANCIA *<br /><br />

*A BIOIMPEDÂNCIA consiste em um exame que avalia com alta precisão e rapidez a composição corporal. Através de uma corrente elétrica imperceptível, é possível avaliar:  músculos, ossos, pele, órgãos, água, fluídos e gordura. 

<p>

<B>Endereço</B><br />
Estrada Caetano Monteiro, 1650 - Loja 102 - Futura Shopping, Pendotiba - Niteroi - Rio de Janeiro
<p><BR /><BR />

<font style="font-size:23px; color:#963e52;">Dicas Saud&aacute;veis</font>
</td>
                    </tr>
                </table>
                <?php
				...
				//$buscarPreparo = mysql_query("SELECT id, descricao FROM dicas ORDER BY id DESC");
				...
				while($array = mysql_fetch_array($buscarPreparo)){
					...
					$img_cardapio_produto_format = substr($descricao,2);
					$img_cardapio_produto_format = "http://".$_SERVER['SERVER_NAME']."/testes/asalight/gerenciador-asalight".$img_cardapio_produto_format;
					
					echo "<tr><td>&nbsp;</td></tr>";
					echo "<tr>";
						echo "<td><a href='$img_cardapio_produto_format' class='lightbox'><img src='$img_cardapio_produto_format' border='0' style='max-width:70px; height:80px;'></a></td>";
					echo "</tr>";
				...
				echo "<div id='paginador'>";
					...
					if($pgs > 1 ) {
						...
						if($menos >= 0) {
							...
							echo "<a title='in&iacute;cio' style='text-decoration:none;' href=http://".$_SERVER['SERVER_NAME']."/site-asalight/dicas-saudaveis.php?p=><strong>in&iacute;cio</strong> </a>";
						}
						...
						if($menos >= 1) {
							...
							echo "<a title='Anterior' style='text-decoration:none;' href=http://".$_SERVER['SERVER_NAME']."/site-asalight/dicas-saudaveis.php?p=".($i)."><strong>anterior</strong> </a>";
						}
					 
						// Listando as paginas
						for($i=$pagina; $i<$pagina+3; $i++) {
							if($i != $pagina) {
								echo " <a href="."http://".$_SERVER['SERVER_NAME']."/site-asalight/dicas-saudaveis.php"."?p=".($i).">$i</a> <font color='#CCCCCC'>|</font>";
						...
						if($mais <= $pgs) {
							...
							echo " <a title='Pr&oacute;xima' style='text-decoration:none;' href="."http://".$_SERVER['SERVER_NAME']."/site-asalight/dicas-saudaveis.php"."?p=".($i)."> <strong>pr&oacute;xima</strong></a>";
						}
						if(($mais >= $pgs) || ($mais <= $pgs)) {
							
							echo " <a title='ultima p&aacute;gina' style='text-decoration:none;' href="."http://".$_SERVER['SERVER_NAME']."/site-asalight/dicas-saudaveis.php"."?p=".($pgs)."> ultima p&aacute;gina</a>";
						}
